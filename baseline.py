...
Original file is located at
    https://colab.research.google.com/drive/1jtrX3GSWHUzp0VUnMxFN6dkPnZlE7Scq
"""
...
# %cd /content/drive/MyDrive/INF573
...
def load_image(img_path,transform_img=True):
    ...
    W, H = image.size
    image = image.resize((700, int(H*700/W)))
...
img_path = 'https://raw.githubusercontent.com/luanfujun/deep-painterly-harmonization/master/data/'
img_index = 16

style_img = load_image('img_bg.jpg')#path should be modified
content_img = load_image('new_pasted_img.jpg')
mask_img = load_image('new_obj_mask.jpg', 0)
dilated_mask_img = load_image('dilated_mask.jpg', 0)

# style_img = load_image(f'{img_path}{img_index}_target.jpg')
# content_img = load_image(f'{img_path}{img_index}_naive.jpg')
# mask_img = load_image(f'{img_path}{img_index}_c_mask.jpg',0)
# dilated_mask_img = load_image(f'{img_path}{img_index}_c_mask_dilated.jpg',0)
...
def vgg_features(data,index=None):
    ...
    for layer_index, layer in model._modules.items():
        data = layer(data)
        if layer_index in layers:#membership test
            features[layers_index[layer_index]]=data
    return features

#extract activation(features) of image
...
#         _, index = cosine_similarity.max(1)
        ...
        index = np.argmax(cosine,1)        # return argmax along axis =1
...
while loop<=epoch:
    def closure():# a function object that remembers values in enclosing scopes even if they are not present in memory.
        optimizer.zero_grad()
        output = list(vgg_features(target_img, index=['16','26','36']).values())
        loss = total_loss(output)
        loss.backward()
        return loss
    optimizer.step(closure)#performs a single optim step, closure is callable
...
#plt.imshow(phase1imgfeatures_mapped_stylefeatures[0].reshape(-1,87).cpu())

# resize style image according to the conv4_1
...
# resize mask image according to the conv4_1
...
#plt.imshow(mask_img_resized.view(-1,87).cpu())
...
#plt.imshow(mapped_phase2.view(-1,87).cpu())
...
def histogram_mask(style_features, mask_features):
    ...
    return torch.cat([torch.histc(masked[i][mask_features>=0.1], n_bins).unsqueeze(0) for i in range(masked.size(0))]).to(device)
...
content_img = (image_pure(content_img).numpy()*255).astype(np.uint8)
phase1_img = (image_pure(phase1_img).numpy()*255).astype(np.uint8)
phase2_img = (image_pure(phase2_img).numpy()*255).astype(np.uint8)
content_img = Image.fromarray(content_img).save("content_img.jpg")
phase1_img = Image.fromarray(phase1_img).save("phase1_img.jpg")
phase2_img = Image.fromarray(phase2_img).save("phase2_img.jpg")

object_img = load_image('img.png')
background_img = load_image('img_bg.png')
fig,(ax1,ax2) = plt.subplots(nrows=1,ncols=2,figsize=(10,10))
ax1.imshow(image_pure(object_img))
ax2.imshow(image_pure(background_img))
plt.show()
